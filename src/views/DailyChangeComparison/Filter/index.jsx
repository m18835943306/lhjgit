...
import { Select, Button, TreeSelect, Form, Radio } from 'antd'
...
const Filter = ({ onQuery, paramsData, setParamsData }) => {
  ...
  useEffect(() => {
    getStatisticalAnalysisrequire(handleParams(json.current))
  }, [])
  const getStatisticalAnalysisrequire = (json) => {
    const params = {
  ...
  const onQueryChange = (v, t, option) => {
    ...
    if (t === 'industry') {
      ...
      json.current.industry_type_id = v
    }
    if (t === 'shiduan') {
      setParamsData((state) => {
  ...
  return (
    <Form layout="inline">
      ...
      <Form.Item label="行业选择">
        ...
        // value={paramsData.industry_type_id-0}
        >
          {industryList &&
